...
namespace Utility
{
    public class Mesh2DColliderMaker
    {
        public static void CreatePolygon2DColliderPoints(MeshFilter filter, PolygonCollider2D polygon)
        {
            var edges = BuildEdgesFromMesh(filter);
            var paths = BuildColliderPaths(edges);
            ApplyPathsToPolygonCollider(polygon, paths);
        }
        #region Helper

        static void ApplyPathsToPolygonCollider(PolygonCollider2D polyCollider, List<Vector2[]> paths)
        {
            if (paths == null)
                return;
        ...
        static Dictionary<Edge2D, int> BuildEdgesFromMesh(MeshFilter filter)
        {
